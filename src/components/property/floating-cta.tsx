...
import { MessageCircle, FileText, Users, BedDouble, Bath } from "lucide-react"
...
const PROPERTY_CONFIG: Record<string, PropertyConfig> = {
    "Kankas House": {
        // priceFrom: 00000,   // <-- set the real direct rate here to switch the price block on
        guests: 12,
        bedrooms: 4,
        baths: 5,
    },
...
export function FloatingCTA({ propertyName, brochureUrl }: { propertyName: string, whatsapp?: string, phone?: string, brochureUrl?: string }) {
    ...
    const facts = [
        cfg.guests ? { icon: Users, label: `Up to ${cfg.guests}`, sub: "Guests" } : null,
        cfg.bedrooms ? { icon: BedDouble, label: String(cfg.bedrooms), sub: "Bedrooms" } : null,
        cfg.baths ? { icon: Bath, label: String(cfg.baths), sub: "Baths" } : null,
    ].filter(Boolean) as { icon: typeof Users; label: string; sub: string }[];
        ...
        <div className="bg-white rounded-lg shadow-lg border border-stayra-gold/10 sticky top-24 overflow-hidden">
            ...
            {facts.length > 0 && (
                <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100">
                    {facts.map((f) => (
                        <div key={f.sub} className="py-3 text-center">
                            <f.icon className="w-4 h-4 mx-auto text-stayra-gold mb-1" />
                            <div className="text-sm font-bold text-stayra-charcoal leading-none">{f.label}</div>
                            <div className="text-[10px] uppercase tracking-wider text-gray-400 mt-1">{f.sub}</div>
                        </div>
                    ))}
                </div>
            )}
